# clop/clop.rb -- clop command-line processing library
# Copyright (c) 2008-2014 William Morgan.
# Copyright (c) 2014 Red Hat, Inc.
# clop is licensed under the same terms as Ruby.
...
## you're using the Clop::options entry.
class CommandlineError < StandardError
end

## Thrown by Parser if the user passes in '-h' or '--help'. Handled
## automatically by Clop#options.
...
## will be handled internally by Clop::options. In this case, only the
...
## Clop::with_standard_exception_handling.
class Parser
  ...
  ## [+:default+] Set the default value for an argument. Without a default value, the hash returned by #parse (and thus Clop::options) will have a +nil+ value for this key unless the argument is given on the commandline. The argument type is derived automatically from the class of the default value given, so specifying a +:type+ is not necessary if a +:default+ is given. (But see below for an important caveat when +:multi+: is specified too.) If the argument is a flag, and the default is set to +true+, then if it is specified on the the commandline the value will be +false+.
  ...
  ## In thise case, Clop assumes that it's not a multi-value argument.
  ...
  ## would be set to the list of subcommands. A subsequent Clop
  ...
  ## Parses the commandline. Typically called by Clop::options,
  ...
  ## The per-parser version of Clop::die (see that for documentation).
  def die(arg, msg = nil)
    if msg
...
## The easy, syntactic-sugary entry method into Clop. Creates a Parser,
...
## If Clop::options doesn't do quite what you want, you can create a Parser
...
##
##   require 'clop'
##   p = Clop::Parser.new do
##     opt :monkey, "Use monkey mode"                     # a flag --monkey, defaulting to false
##     opt :goat, "Use goat mode", :default => true       # a flag --goat, defaulting to true
##   end
##
##   opts = Clop::with_standard_exception_handling p do
##     o = p.parse ARGV
##     raise Clop::HelpNeeded if ARGV.empty? # show help screen
##     o
##   end
##
...
##   end
##
##   Clop::die "need at least one filename" if ARGV.empty?
def die(arg, msg = nil)
  if @last_parser
    @last_parser.die arg, msg
  else
    raise ArgumentError, "Clop::die can only be called after Clop::options"
  end
end
...
##   end
##
##   Clop::educate if ARGV.empty?
def educate
  if @last_parser
    @last_parser.educate
    exit
  else
    raise ArgumentError, "Clop::educate can only be called after Clop::options"
  end
end
